...
export function VehicleSectionCard({ icon: Icon, title, subtitle, children, collapsible = false, defaultOpen = true, badge }: Props) {
  ...
  return (
    <div className="rounded-2xl border border-surface-border bg-white shadow-sm overflow-visible">
      ...
      {(!collapsible || open) && (
        <div className="px-6 pb-6 pt-1 border-t border-surface-border/60">{children}</div>
      )}
    </div>
  )
}
